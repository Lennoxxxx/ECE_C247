...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache = (sample_mean, sample_var, x, x_norm, eps, gamma, beta)
...
def batchnorm_backward(dout, cache):
    ...
    dxhat = dout*gamma
    dvar = np.sum((-0.5*(x-mean)*dxhat/np.power((var+eps),1.5)),axis = 0)
    dmean = -np.sum(dxhat,axis = 0)/np.sqrt(var+eps) - 2*dvar*np.sum((x-mean),axis = 0)/x.shape[0]
    dx = dxhat/np.sqrt(var+eps) + 2*dvar*(x-mean)/x.shape[0] +dmean/x.shape[0]
