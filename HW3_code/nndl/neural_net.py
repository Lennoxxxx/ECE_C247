...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    loss_softmax = np.sum(np.log(np.sum(np.exp(scores),axis = 1))-scores[range(N),y])/N
    
    L2 = lambda x: np.sum(x**2)
    loss_r2 = 0.5*(L2(W1)+L2(W2))
    
    loss = loss_softmax+reg*loss_r2
    ...
    dLdscores = (np.exp(scores)/(np.sum(np.exp(scores),axis = 1)[:,np.newaxis]+np.zeros(scores.shape[1])))
    dLdscores[range(N),y] -= 1 
    dLdscores /= N 
    
    grads['W2'] = dLdscores.T.dot(h1a) + reg*W2
    grads['b2'] = np.sum(dLdscores,axis = 0)
    dLdh1a = dLdscores.dot(W2)
    
    dLdh1 = dLdh1a*(h1>0)
    
    grads['W1'] = dLdh1.T.dot(X) + reg*W1
    grads['b1'] = np.sum(dLdh1,axis = 0)
